    ...
    <div class="default">
        <div v-if="view" class="container">
            ...
            <p class="center"><span class='weight'>AUTOBOX SIDI YAHIA</span>
               ...
               <span><barcode id='sps' :value="detail.qr_vage"></barcode></span>
        ...
        <div v-if="!view" class="container">
            <router-link to='/vache' style='margin-bottom:10px;' class='btn btn-warning' exact> <ic icon="arrow-left" /> </router-link>
            <div class="card">
...
export default {
    ...
    methods : {
        genTicket()
        {
            var run = axios.post(this.link() + 'caise1',this.data)
            run.then(e=>{
                if(e.data.length > 0)
                {
                         this.details = e.data
